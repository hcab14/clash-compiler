...
instance (FromAst a) => FromAst (Vec n a) where
  fromTerm (stripTicks -> term)
    ...
    = do traceM ("fromTerm.LVec: NIL")
         pure (unsafeCoerce Nil)

    | Data _dc `TyApp` _nTy `TyApp` _aTy `TyApp` _eTy `App` _co `App` x `App` y <- term
    = do traceM ("fromTerm.LVec: CONS")
         a  <- fromTerm @a x
         traceM ("fromTerm.LVec: GOT a")
         as <- fromTerm @(Vec (n - 1) a) y
         traceM ("fromTerm.LVec: GOT as")
...
mkPrimIndex ty = do
  ...
  let pTy = mkPolyFunTy (mkTyConApp iNm [VarTy n])
              [Left n, Right naturalPrimTy, Right integerPrimTy]
